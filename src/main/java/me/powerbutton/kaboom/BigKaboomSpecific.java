...
public class BigKaboomSpecific implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("bigspecific")) {
            if (!sender.hasPermission("kaboomspecific.use")) {
                ...
                Player players = Bukkit.getPlayerExact(args[0]);
                Iterator var6 = Bukkit.getOnlinePlayers().iterator();

                while(var6.hasNext()) {
                    Player things = (Player)var6.next();
                    if (players == null) {
                        sender.sendMessage(ChatColor.RED + "This player does not exist or is not online!");
                    } else {
                        World world = (World)Bukkit.getWorlds().get(2);
                        ...
                        Bukkit.getScheduler().runTaskLater(Kaboom.getInstance(), () -> {
                            things.sendMessage(ChatColor.RED + "Fall damage has been turned §lON. " + ChatColor.AQUA + "Now everything is back to normal!");
                        }, 1400L);
                        Bukkit.getScheduler().runTaskLater(Kaboom.getInstance(), () -> {
                        ...
                        things.sendMessage(ChatColor.GREEN + "Fall Damage has been turned §lOFF. " + players.getName() + " §b§l is experiencing /bigkaboom at this moment! Fall damage will be turned back on in 1:10 minutes!");
                    }
                }
            } else if (args.length >= 0) {
                sender.sendMessage(ChatColor.RED + "[kaboom.] Missing arguments! The correct usage of this command is /bigspecific <playername>");
            }
        }
